...
program.
  ...
  .option('-g, --gold <string>', 'Gold card count', '1')
  .option('-s, --silver <string>', 'Silver card count', '2')
  .option('-b, --bronze <string>', 'bronze card count', '20')
  .action(async (directory, cmd) => {
    const { keypair, env, gold, silver, bronze } = cmd.opts();

    const walletKeyPair = loadWalletKey(keypair);
    const anchorProgram = await loadAnchorProgram(walletKeyPair, env);

    const generator = Keypair.generate();

    await anchorProgram.rpc.initializeCards(
      new BN(gold), new BN(silver), new BN(bronze),
      {
        accounts: {
          data: generator.publicKey,
    ...
    console.log(`create_random_generator Done: ${generator.publicKey.toBase58()}`);
});


program.
  command('create_branden_contract')
  .option(
    '-e, --env <string>',
    'Solana cluster env name',
    'devnet', //mainnet-beta, testnet, devnet
  ...
  .option('-t, --total-count <string>', 'Total mint count', '10000')
  .option('-p, --presale-count <string>', 'Presale mint count', '2000')
  .option('-d, --presale-date <string>', 'Presale mint date', '04 Dec 1995 00:12:00 GMT')
  .action(async (directory, cmd) => {
    const { keypair, env, totalCount, presaleCount, presaleDate } = cmd.opts();

    const millisSinceEpoch = presaleDate ? Date.parse(presaleDate) : new Date().getMilliseconds();
    const walletKeyPair = loadWalletKey(keypair);
    const anchorProgram = await loadAnchorProgram(walletKeyPair, env);
    ...
    await anchorProgram.rpc.initializeContract(
      new BN(totalCount), new BN(presaleCount), new BN(millisSinceEpoch),
      {
        accounts: {
          data: contract.publicKey,
          user: walletKeyPair.publicKey,
          systemProgram: anchor.web3.SystemProgram.programId,
        },
        signers: [contract],
    ...
    console.log(`create_presale Done: ${contract.publicKey.toBase58()}`);
});
